...
import { finiteNumber,hasMeaningfulVariance } from './numeric';
...
export const numericCorrelation=(a:ColumnProfile,b:ColumnProfile)=>{const pairs:[number,number][]=[];a.values.forEach((value,i)=>{const x=finiteNumber(value),y=finiteNumber(b.values[i]);if(x!==undefined&&y!==undefined)pairs.push([x,y])});if(pairs.length<3||!hasMeaningfulVariance(pairs.map(([x])=>x))||!hasMeaningfulVariance(pairs.map(([,y])=>y)))return undefined;const ax=pairs.reduce((sum,[x])=>sum+x,0)/pairs.length,ay=pairs.reduce((sum,[,y])=>sum+y,0)/pairs.length;let numerator=0,dx=0,dy=0;for(const [x,y] of pairs){numerator+=(x-ax)*(y-ay);dx+=(x-ax)**2;dy+=(y-ay)**2}return Math.abs(numerator/Math.sqrt(dx*dy))};
...
export function suggestCharts(profiles:ColumnProfile[]):ChartSpec[]{
 ...
 if(target){
  ...
  nums.filter(x=>x.name!==target.name).map((profile,index)=>({profile,index,strength:numericCorrelation(profile,target)})).filter(item=>item.strength!==undefined).sort((a,b)=>b.strength!-a.strength!||a.index-b.index).slice(0,3).forEach(({profile},index)=>out.push({id:`target-scatter-${index}`,title:`${displayName(profile.name,true)} vs ${displayName(target.name)}`,subtitle:`Relationship with ${displayName(target.name)}`,kind:'scatter',x:profile.name,y:target.name}));
 ...
 if(!target&&out.length<4&&nums[1]&&numericCorrelation(nums[0],nums[1])!==undefined)out.push({id:'scatter',title:`${displayName(nums[0].name,true)} vs ${displayName(nums[1].name)}`,subtitle:'Relationship between numeric fields',kind:'scatter',x:nums[0].name,y:nums[1].name});
