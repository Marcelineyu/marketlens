...
export function detectType(values:unknown[], name=''):ColumnType{
 ...
 if(numeric>.95){const nums=strings.map(Number);if(ratio>.9&&indexName(name)&&sequential(nums))return 'identifier';if(ratio>.96&&present.length>20&&/(^id$|_id$|code|number$)/i.test(name))return 'identifier';return 'numeric'}
 const dates=strings.filter(v=>/[-/]/.test(v)&&!Number.isNaN(Date.parse(v))).length/present.length;
